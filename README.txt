...
<header>
  ...
  <div class="tag">V42 | volume científico SoulFit+</div>
</header>
...
function init(){
  letras.forEach(l=>{if(!treinos[l].length)treinos[l]=Array.from({length:10},linhaVazia)})
...
function aplicarTreinoTexto(txt){
  ...
  letras.forEach(l=>{
    treinos[l]=Array.from({length:10},(_,i)=>parsed[l][i]||linhaVazia());
  });
...
async function gerarComIA(){
  ...
  try{
    ...
    aplicarTreinoTexto(json.treino);
    statusIA('Treino gerado com IA, volume ajustado por nível e aplicado na área de edição e na ficha de impressão. Revise antes de imprimir.','ok');
  }catch(e){
...
function gerarEvolucao(){
  letras.forEach(l=>treinos[l]=Array.from({length:10},linhaVazia));
  let base;
  ...
  Object.keys(base).forEach(l=>{
    treinos[l]=Array.from({length:10},(_,i)=>{
      const x=base[l][i];
